...
const responsive = {
    ...
    return (
      <section className="w-4/5 container mx-auto max-w-7x1 " data-scroll-section>
        <h2 className="mx-auto flex flex-col uppercase anton text-2xl text-trueGray-700">latest articles</h2>
            ...
            { data.allContentfulBlogPost.edges.map((edge, i) => (
                <Link className=" xl:w-1/3 md:w-1/2 m-8" key={edge.node.id} to={`/blog/${edge.node.slug.toLowerCase().trimStart().replace(/\s/g, '-')}/`}>
                  <div className="bg-white p-6 m-4 rounded-lg card-shadow">
                      <h2 className="text-lg text-gray-900 font-medium title-font mb-4 h-14 line-clamp-2">{edge.node.title}</h2>
